...
export const handler: CloudFrontRequestHandler = async (event) => {
    ...
    try {
        ...
        try {
            ...
            const newTokens = await refreshToken(cookies.refreshToken, config.userPoolAppId);
        ...
        const response = {
            ...
            headers: {
                location: [
                    {
                        key: 'location',
                        value: `https://${config.cloudFrontDomain}/auth/sign-in`,
                    },
                ],
            },
        };
 ...
 * @returns
 */
async function refreshToken(refreshToken: string, client_id: string): Promise<any> {
    try {
 ...
 * @returns
 */
function generateCookieHeaders(idToken: string, accessToken: string, refreshToken: string, client_id: string) {
    ...
    Object.assign(cookies, {
        [cookieNames.lastUserKey]: `${tokenUserName}; ${cookieSettings.idToken} Expires=${expire.toUTCString()}`,
        [cookieNames.userDataKey]: `${encodeURIComponent(userData)}; ${
            cookieSettings.idToken
        } Expires=${expire.toUTCString()}`,
        'amplify-signin-with-hostedUI': `false; ${cookieSettings.accessToken} Expires=${expire.toUTCString()}`,
    });

    // Set JWTs in the cookies
    cookies[cookieNames.idTokenKey] = `${idToken}; ${cookieSettings.idToken} Expires=${expire.toUTCString()}`;
    if (accessToken) {
